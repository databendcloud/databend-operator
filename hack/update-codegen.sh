...
echo "Generating OpenAPI specification for databendlabs.io/v1"
go run ${OPENAPI_PKG}/cmd/openapi-gen \
  --go-header-file "${DATABEND_OPERATOR_ROOT}/hack/boilerplate.go.txt" \
  --output-pkg "${DATABEND_OPERATOR_PKG}/pkg/apis/databendlabs.io/v1" \
  --output-dir "${DATABEND_OPERATOR_ROOT}/pkg/apis/databendlabs.io/v1" \
  --output-file "zz_generated.openapi.go" \
  --report-filename "${DATABEND_OPERATOR_ROOT}/hack/violation_exception_v1.list" \
  "${DATABEND_OPERATOR_ROOT}/pkg/apis/databendlabs.io/v1"
